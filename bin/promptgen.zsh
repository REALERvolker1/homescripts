...
if command -v conda &>/dev/null; then
    condafunc_name='__vlkprompt::precmd::conda'
    condafunc="$condafunc_name() {
    if [[ -n \${CONDA_DEFAULT_ENV-} ]]; then
        psvar[${index[conda]}]=1
        __vlkprompt_internal[conda_str]=\$CONDA_DEFAULT_ENV
    fi
}"
    condafunc_name="'$condafunc_name'"
fi

if python3 -m venv -h >/dev/null; then
    venvfunc_name='__vlkprompt::precmd::venv'
    venvfunc="$venvfunc_name() {
    if [[ -n \${VIRTUAL_ENV-} ]]; then
        psvar[${index[venv]}]=1
        __vlkprompt_internal[venv_str]=\${VIRTUAL_ENV:t}
    fi
}"
    venvfunc_name="'$venvfunc_name'"
fi

(
cat << EOF
[[ \${VLKZSH_SAFEMODE:-1} -eq 1 || -n \${VLKPROMPT_SKIP-} ]] && return
