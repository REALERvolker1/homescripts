...
if ( defined $ENV{"PSAPL_DEBUGFILE"} ) {
    warn "Printing output to debugfile at $ENV{'PSAPL_DEBUGFILE'}\n";
    open( my $debugfile, ">", $ENV{"PSAPL_DEBUGFILE"} )
      or die "Could not open file: $!";
    print $debugfile $output_str;
    close $debugfile;
}
...
my $pidstats = `$pidstat '$pid'`;
chomp($pidstats);

my $action =
`echo '$action_str' | fzf --ansi --preview-window='down,50%' --preview=\"echo '$pidstats'\"`;
$action =~ s/^([0-9]+).*\n/$1/;
