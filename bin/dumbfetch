...
if ( defined $ENV{'CONTAINER_ID'} ) {
    ...
    #     fi
    # `;
    # nvidia-persistenced --version | grep -oP 'version\s\K[^\s]+'
    #my $check_rpm = system("command -v rpm >/dev/null 2>&1");
        ...
        #}
    #$driver_content =~ s/\n//g;
    # im feeling really lazy rn
    $driver_content = `nvidia-persistenced --version | grep -oP 'version\\s\\K[^\\s]+'`;
    chomp($driver_content);
...
my @cmds =
  ( $ENV{'SHLVL'}, $uptime, $ENV{'TERM'}, $disks, $driver_content, $kernel, );
...
# print "\e[0;38;5;${box_color}m│\e[0;${user_color}m $user[$count] \e[0;38;5;${box_color}m│\e[0;$cmd_keys[$count] \e[1m$_ \e[0;38;5;${box_color}m│\e[0m\n";
    print
"${box_color}│\e[0;${user_color}m $user[$count] ${box_color}│\e[0;$cmd_keys[$count]   \e[1m$content ${box_color}│\e[0m\n";
    $count += 1;
}
$boxtop =~ s/╭/╰/g;
$boxtop =~ s/┬/┴/g;
