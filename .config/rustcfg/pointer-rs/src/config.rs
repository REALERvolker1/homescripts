...
pub fn detect_touchpads(devices: &Vec<Mouse>) -> Res<Mouse> {
    if_chain::if_chain! {
        if let Some(ident) = CONFIG.touchpad_identifier.as_ref();
        let ident_lowercase = ident.to_ascii_lowercase();
        if let Some(n) = devices.iter().filter(|n| n.name.contains(&ident_lowercase)).next();
        then {
            return Ok(n.clone());
        }
        else {
